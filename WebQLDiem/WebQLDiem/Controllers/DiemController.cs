        ...
        public ActionResult Edit(BaiTap_HocVien model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    SendThongBaoTaiKhoan(obj.HocVien.MaTaiKhoan.Value, "Bài tập [" + obj.BaiTap.TenBaiTap + " - " + obj.BaiTap.LopHoc.TenLopHoc + "] của " + obj.HocVien.TenHocVien + " đã được chấm điểm bởi " + cookie.Value.ToString(), "Bài tập [" + obj.BaiTap.TenBaiTap + " - " + obj.BaiTap.LopHoc.TenLopHoc + "] của " + obj.HocVien.TenHocVien + " đã được chấm điểm bởi " + cookie.Value.ToString());
                }
                catch
                {
