...
{
    [GiaoVienAuthorize]
    public class LopHocGiaoVienController : BaseController
    {
        // GET: LopHoc
        public ActionResult Index()
        {
            var gv = Db.GiangViens.FirstOrDefault(x => x.MaTaiKhoan == MaTaiKhoan);
            var list = Db.LopHocs.Where(x => x.NguoiTao == MaTaiKhoan || x.GiangViens.Any(y => y.MaGiangVien == gv.MaGiangVien)).ToList();
            return View(list);
        }
        ...
        public ActionResult Edit(int id)
        {
            ...
            {
                var gv = Db.GiangViens.FirstOrDefault(x => x.MaTaiKhoan == MaTaiKhoan);
                var model = Db.LopHocs.FirstOrDefault(x => x.MaLopHoc == id && ( x.NguoiTao == MaTaiKhoan || x.GiangViens.Any(y => y.MaGiangVien == gv.MaGiangVien)));
                return View(model);
            }
            catch
            {
        ...
        public ActionResult Edit(LopHoc model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var gv = Db.GiangViens.FirstOrDefault(x => x.MaTaiKhoan == MaTaiKhoan);
                    var obj = Db.LopHocs.FirstOrDefault(x => x.MaLopHoc == model.MaLopHoc && (x.NguoiTao == MaTaiKhoan || x.GiangViens.Any(y => y.MaGiangVien == gv.MaGiangVien)));
        ...
        public ActionResult XoaChuDe(XoaChuDeMeta model)
        {
            try
            {
                ...
                if (obj.ChuDes.Count(x => x.MaChuDe == model.MaChuDe && x.NguoiTao == MaTaiKhoan) == 0)
                {
        ...
        public ActionResult ThongTinLopHoc(int malop)
        {
            ...
            var gv = Db.GiangViens.FirstOrDefault(x => x.MaTaiKhoan == MaTaiKhoan);
            var lophoc = Db.LopHocs.FirstOrDefault(x => x.MaLopHoc == malop && (x.NguoiTao == MaTaiKhoan || x.GiangViens.Any(y => y.MaGiangVien == gv.MaGiangVien)));
            if (lophoc != null)
            {
                foreach (var item in lophoc.BaiTaps.Where(x => x.TrangThai >= (int)TrangThaiBaiTap.GiaoBaiTap && x.NguoiTao == MaTaiKhoan).ToList())
                {
                ...
                foreach (var item in lophoc.TaiLieux.Where(x => x.TrangThai >= (int)TrangThaiTaiLieu.GiaoTaiLieu && x.NguoiTao == MaTaiKhoan).ToList())
                {
                    listThongTin.Add(new ThongTinLopHop
                    {
                        Id = item.MaTaiLieu,
